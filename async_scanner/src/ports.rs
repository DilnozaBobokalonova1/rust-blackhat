...
pub async fn scan_ports(concurrency: usize, subdomain: Subdomain) -> Subdomain {
    ...
    //Concurrent stream method 3: using rust channels
    let (input_tx, input_rx) = mpsc::channel::<u16>(concurrency);
    let (output_tx, output_rx) = mpsc::channel::<Port>(concurrency);

    //all of the ports have been sent by the transmitter upon completion of spawn
    //note that spawn will be running in the background while the next functions
    //are also running
